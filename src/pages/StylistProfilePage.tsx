...
const StylistProfilePage = () => {
  ...
  const backLink = location.state?.from || "/ai-recommendation";
  const backLabel = location.state?.label ? `Back to ${location.state.label}` : "Back to AI Recommendation";
  ...
  if (!stylist) return (
    <div className="py-20 text-center min-h-screen flex items-center justify-center bg-background">
      ...
      >
        ...
        <Link to={backLink} className="bg-accent text-primary font-semibold inline-flex items-center gap-2 px-6 py-3 rounded-xl hover:bg-accent/90 transition-all duration-300 shadow-lg hover:shadow-accent/25 group">
          {backLabel} <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
        </Link>
      </motion.div>
    </div>
  );
    ...
    <div className="relative bg-background">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-primary via-primary to-primary/95 h-[450px] md:h-[500px] pb-4 rounded-b-3xl shadow-2xl">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-8">
          <Link to={backLink} className="inline-flex items-center gap-2 text-sm text-white/80 hover:text-white transition-colors bg-black/20 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 shadow-lg hover:bg-black/30">
              ...
              >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  {stylist.portfolio.map((item, i) => (
                    <div key={i} className="group relative rounded-xl overflow-hidden aspect-square border border-border/50 hover:border-accent/30 transition-all duration-300">
                      <Zoom>
                        <img 
                          src={item.image} 
                          alt={item.name} 
