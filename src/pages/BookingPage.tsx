...
import { SelectedAIStyleDetails, SelectedStyleData } from "@/components/ai-recommendation";
import { 
...
const BookingPage = () => {
  ...
  const isAIRecommendation = params.get("ai") === "true";
  const [aiStyleData, setAIStyleData] = useState<SelectedStyleData | null>(null);

  // Load AI recommendation data from localStorage
  useEffect(() => {
    if (isAIRecommendation && user?.id) {
      const stored = localStorage.getItem(`ai_booking_${user.id}`);
      if (stored) {
        ...
        setAIStyleData({
          ...
          hairSafetyLevel: data.hairSafetyLevel,
          frontImage: data.frontImage,
          backImage: data.backImage,
        });
      }
    }
  }, [isAIRecommendation, user?.id]);
  ...
  const getInitialStep = () => {
    switch (bookingType) {
      case 'ai-recommended': return 1; // Show AI style details then select date
  ...
  const stepTitles = bookingType === 'ai-recommended'
    ? ["AI Style Details", "Select Date & Time", "Choose Stylist", "Review & Request"]
    : bookingType === 'instant-match'
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
        ...
        >
          ...
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            {bookingType === 'ai-recommended' && "Review your AI-recommended braid style and select your preferred stylist"}
        ...
        {/* AI Style Details Section */}
        {bookingType === 'ai-recommended' && aiStyleData && (
          <SelectedAIStyleDetails data={aiStyleData} />
        )}
