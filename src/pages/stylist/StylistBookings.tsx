...
import ViewCustomerProfileModal from "@/components/ViewCustomerProfileModal";
import { 
  ...
  Shield, TrendingUp, Users, FileText, Heart
} from "lucide-react";
...
const StylistBookings = () => {
  ...
  const [selectedCustomerBooking, setSelectedCustomerBooking] = useState<Booking | null>(null);
  const { user } = useAuth();
                            ...
                            <div className="border-t border-border/50 pt-5 space-y-5">
                              ...
                              {b.bookingType === 'ai-recommended' && b.aiRecommendation && (
                                <motion.div
                                  ...
                                  className="bg-gradient-to-br from-purple-500/5 to-purple-500/0 rounded-xl p-5 border border-purple-500/10"
                                >
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                      <h5 className="text-sm font-medium text-muted-foreground mb-2 text-center">Customer Photo (Front)</h5>
                                      {b.aiRecommendation.frontImage ? (
                                        <img src={b.aiRecommendation.frontImage} alt="Customer's front view" className="rounded-lg w-full aspect-square object-cover" />
                                      ) : <p className="text-xs text-center text-muted-foreground">Not provided</p>}
                                    </div>
                                    <div>
                                      <h5 className="text-sm font-medium text-muted-foreground mb-2 text-center">Customer Photo (Back)</h5>
                                      {b.aiRecommendation.backImage ? (
                                        <img src={b.aiRecommendation.backImage} alt="Customer's back view" className="rounded-lg w-full aspect-square object-cover" />
                                      ) : <p className="text-xs text-center text-muted-foreground">Not provided</p>}
                                    </div>
                                    <div>
                                      <h5 className="text-sm font-medium text-muted-foreground mb-2 text-center">AI Suggested Style</h5>
                                      <img src={b.aiRecommendation.styleImage} alt={b.aiRecommendation.styleName} className="rounded-lg w-full aspect-square object-cover" />
                                  ...
                                  {b.aiRecommendation.healthReport && (
                                    <div className="mt-4 pt-4 border-t border-purple-500/20">
                                      ...
                                      <div className="text-sm text-muted-foreground space-y-2">
                                        <p><strong>Health Score:</strong> {b.aiRecommendation.healthReport.hairHealthScore}/100</p>
                                        <p><strong>Risk Level:</strong> {b.aiRecommendation.healthReport.riskLevel}</p>
                                        <p><strong>Recommendations:</strong></p>
                                        <ul className="list-disc list-inside pl-4">
                                          {b.aiRecommendation.healthReport.recommendations.map((rec, i) => <li key={i}>{rec}</li>)}
                                        </ul>
                                      </div>
                                    </div>
                                  )}
      ...
      <ViewCustomerProfileModal
        ...
        customer={
          selectedCustomerBooking
            ? {
                id: selectedCustomerBooking.customerId,
                name: selectedCustomerBooking.customerName,
                email: selectedCustomerBooking.customerName.toLowerCase().replace(' ', '.') + '@email.com',
                phone: '(555) 123-4567',
                avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150&h=150&fit=crop',
                location: 'London, UK',
                hairType: 'Curly',
                selectedBraidStyle: selectedCustomerBooking.service || 'Box Braids',
                selectedBraidImage: 'https://images.unsplash.com/photo-1596178060671-7a80dc8059ea?w=300&h=300&fit=crop',
                healthScore: 85,
                notes: selectedCustomerBooking.notes || '',
                bookingDetails: {
                  date: selectedCustomerBooking.date,
                  time: selectedCustomerBooking.time,
                  service: selectedCustomerBooking.service,
                  price: selectedCustomerBooking.price,
                },
              }
            : null
        }
