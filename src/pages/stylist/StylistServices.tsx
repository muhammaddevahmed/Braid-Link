...
const StylistServices = () => {
  ...
  const addService = () => {
    ...
    const minPrice = getSelectedHairstyle(newServiceForm.categoryId, newServiceForm.hairstyleId)?.startingPrice || 0;
    if (Number(newServiceForm.price) < minPrice) {
      setSaveStatus("error");
      setTimeout(() => setSaveStatus("idle"), 3000);
      return;
    }
  ...
  const updatePrice = (serviceId: string, newPrice: number) => {
    ...
    const hairstyle = getSelectedHairstyle(service.categoryId, service.hairstyleId);
    const minPrice = hairstyle?.startingPrice || 0;
  ...
  const selectedHairstyleForNew = newServiceForm.hairstyleId ? getSelectedHairstyle(newServiceForm.categoryId, newServiceForm.hairstyleId) : null;
  const newMinPrice = selectedHairstyleForNew?.startingPrice || 0;
  const isNewPriceValid = !newServiceForm.price || Number(newServiceForm.price) >= newMinPrice;
      ...
      {services.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {services.map((svc, idx) => {
            ...
            const editMinPrice = selectedHairstyle?.startingPrice || 0;
            const isEditPriceValid = isEditing && (!editPriceValue || Number(editPriceValue) >= editMinPrice);
                                    ...
                                    <div className="flex flex-col gap-2 items-end">
                                      <div className="flex gap-2 items-center">
                                        <input
                                          type="number"
                                          className={cn(
                                            "w-24 px-3 py-1.5 rounded-lg text-sm border-2 bg-background text-primary font-semibold",
                                            !isEditPriceValid ? "border-rose-500" : "border-accent"
                                          )}
                                        ...
                                        <motion.button
                                          ...
                                          disabled={!isEditPriceValid || !editPriceValue}
                                          className="px-3 py-1.5 text-xs font-semibold bg-accent text-primary rounded-lg hover:bg-accent/90 transition disabled:opacity-50"
                                        >
                                          Save
                                        </motion.button>
                                        <motion.button
                                      ...
                                      {!isEditPriceValid && (
                                        <p className="text-xs text-rose-500">
                                          Price cannot be less than the admin starting price: £{editMinPrice}
                                        </p>
                                      )}
                                    </div>
                                  ) : (
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              {newMinPrice > 0 && (
                <span className="text-xs font-medium text-muted-foreground">
                  Min starting price: £{newMinPrice}
                </span>
              )}
            </div>
            <input
              type="number"
              className={cn(
                "w-full px-4 py-3.5 rounded-xl border bg-background text-sm focus:outline-none focus:ring-2 transition-all placeholder:text-muted-foreground/50",
                !isNewPriceValid
            ...
            {!isNewPriceValid && (
              <p className="text-xs text-rose-500 flex items-center gap-1 mt-1">
                ...
                Price cannot be less than the admin starting price: £{newMinPrice}
              </p>
            )}
          </div>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            onClick={addService}
            disabled={!isNewPriceValid || !newServiceForm.price || !newServiceForm.hairstyleId}
            className="w-full bg-gradient-to-r from-accent to-accent/90 text-primary font-bold py-4 rounded-xl flex items-center justify-center gap-2 shadow-lg hover:shadow-xl transition-all text-sm mt-4 group disabled:opacity-50 disabled:cursor-not-allowed"
          >
